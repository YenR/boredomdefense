...
public class SVMOde extends JFrame{
...
//    static boolean  dodraw = true;
    ...
    SVMOde()
    {
        ...
        setTitle(Global.title + " - Survivial Mode");
        setResizable(false);
    ...
    public void drawmyline(int i, int j, int zi, int zj, Graphics2D g2) // Tower at Coord i,j attacks Enemy at Coord zi, zj
    {
        if(plates[zi][zj].e != null)
        {
            ...
            plates[i][j].t.nextShot = (plates[i][j].t.type == 2)?10:(plates[i][j].t.type == 3)?40:5;

            plates[zi][zj].e.hp -= ( (plates[i][j].t.type == 2)?(20 + plates[i][j].t.level * 10):(plates[i][j].t.type == 3)?0:(5 + 2*plates[i][j].t.level));

            if(plates[i][j].t.type == 3)    // -1/2 hp
            {
                plates[zi][zj].e.hp -= plates[zi][zj].e.hp/(2+(5-plates[i][j].t.level));
                plates[zi][zj].e.hp++;
            }
...
//            }
            ...
            {
                int geld = (100 + plates[zi][zj].e.pl * 4) * 100 / (100 + plates[zi][zj].e.pl);
                resource += (geld>200)?200:geld;
                plates[zi][zj].e = null;
            }
...
//                        plates[i][j].setBackground(new Color(0,0,0));
            ...
            timer2.scheduleAtFixedRate(new EnemyTask(), 5000, 4000);
                    ...
                    if(plates[i][j].t != null)
                        {
                            try
                            {
                                 ...
                                 */
                                if((plates[i][j].t.type == 2 )&& plates[i][j].t.nextShot == 0)    // Green Tower
                                {
                                    /*
                                     *                             o 
                                     *  Level 0 Shot     ->       oXo
                                 ...
                                 *  Red Tower
                                 */
                                else if((plates[i][j].t.type == 4 || plates[i][j].t.type == 3) && plates[i][j].t.nextShot == 0)  
                                {
                                    /*
                                     *                             o 
                                     *  Level 0 Shot     ->       oXo
...
//            
            ...
            g2d.drawString("Money: " + resource, 330, hi*30 + 35);
            //g2d.drawString("HP: " + hp, 420, hi*30 + 35);
            g2d.drawString("Wave: " + wave, 450, hi*30 + 35);
    ...
    class EnemyTask extends TimerTask {
        ...
        public void run() {
            ...
            for(int i=0; i<wi; i++)
            {
                for(int j=0; j<hi; j++)
                {
                    if(plates[i][j].e != null || plates[i][j].t != null )
                        cnt++;
                }
            }
